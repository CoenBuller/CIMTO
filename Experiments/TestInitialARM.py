...
def RunInitialARM(dart_config: Config, 
    ...
    for init_arm in init_arm_its:
        dart_config.init_arm_iters = init_arm

        for n in n_projections:
            dart_config.n_angles = n

            save_dir = os.path.join(dart_config.save_dir, "InitialARM", f"{init_arm}", f"projections_{n}")
            os.makedirs(save_dir, exist_ok=True)

            for phantom_gen in phantoms:
                file_name = PHANTOM_NAMES[phantom_gen]
                print(f"Running: Initial ARM Iters: {init_arm} | N projections: {n} | Phantom: {PHANTOM_NAMES[phantom_gen]}")
                results = {}
...
if __name__ == "__main__":
    ...
    dart_cfg = Config()
    RunInitialARM(dart_config=dart_cfg, 
                  init_arm_its = INIT_ARM_ITS, 
